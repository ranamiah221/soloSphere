...
const JobDetails = () => {
    ...
    event.preventDefault()
     if (buyer?.email === user?.email){
        return toast.error("Sorry you can not bid this job")
    }
    ...
    return (
      <div className='flex flex-col md:flex-row justify-around gap-5 my-10  items-center min-h-[calc(100vh-306px)] md:max-w-screen-xl mx-auto '>
        ...
        <div className='flex-1  px-4 py-7 bg-white rounded-md shadow-md md:min-h-[400px]'>
          ...
          <div>
            ...
            <div className='flex items-center gap-5'>
              <div>
                <p className='mt-2 text-sm  text-gray-600 '>Name: {buyer?.buyerName}</p>
                <p className='mt-2 text-sm  text-gray-600 '>
                  Email: {buyer?.email}
                </p>
              </div>
              <div className='rounded-full object-cover overflow-hidden w-14 h-14'>
                <img src={buyer?.buyerImg} alt='' />
              </div>
            </div>
        ...
        {/* Place A Bid Form */}
        <section className='p-6 w-full  bg-white rounded-md shadow-md flex-1 md:min-h-[400px]'>
